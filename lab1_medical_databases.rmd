...
library(RCurl)          # Library to read CSV from Github (online repository)
...
raw.data <- read_csv('https://raw.githubusercontent.com/Sai97-Ravi/CSCI6410/Lab_1/2016_05v2_VitoriaAppointmentData.csv', col_types='fffTTifllllflf')

#Printing thr dimensions (number of rows and columns in the data)
dim(raw.data)
```
...
**One plausible reason is due to unforeseen circumstanses for a patient on their appointment day. For example, based on the above code, we see that older patients (greater than 60 years of age) are not able to attend their appointment, in-spite of receiving SMS reminder.**
...
**Time to reach the healthcare center depends on the location of the neighborhood and any unforseen circumstances while travelling to the healthcare center such as detour, would account for attendance during the scheduled appointment.**
...
[1] [Correlation and Simple Linear Regression](https://pubs.rsna.org/doi/abs/10.1148/radiol.2273011499), Kelly H. Zou, Kemal Tuncali, and Stuart G. Silverman, Radiology 2003 227:3, 617-628
