...
**Answer: The given data set consists of the following 6 variables:**

1.  **`note_id`: Uniquely identifies the textual note for each patient listed.**

2.  **`description`: A high level textual description of medical procedure or problem experienced by a patient.**

3.  **`medical_specialty`: The high level text description is mapped to particular medical specialty (out of the 40 given medical specialties)**

4.  **`sample_name`: It indicates the patient's medical problem including lab sample, test or medical procedure.**

5.  **`transcription`: Further, the high level text description is transcribed to a lower level description comprising of further details of the medical problem including the medical terms.**

6.  **`keywords`: Keywords consists of the cluster of the possible words that are constructed based on the relevance to the patient's `description`, `medical_specialty` and `sample_name` (for each note).**
...
**Using `'sentences'` token from `tidytext::unnest_tokens()` to create `token.of.sentences`**

```{r}
token.of.sentences <- analysis.data %>% tidytext::unnest_tokens(sentence, transcription, token = "sentences", to_lower = TRUE)
```

**Extracting unique sentences from each specialty from the `token.of.sentences`**

```{r}
token.of.sentences %>% dplyr::group_by(medical_specialty) %>% dplyr::distinct(sentence) %>% dplyr::summarise(n=dplyr::n())
```
...
**Answer: Using a general purpose lemmatizer for medical data will cause either incorrect or misleading lemmatization of the terms within the medical data. As such, medical data consists of complex data types, terms and notations. Moreover, medical data also consists of terminologies and phrases which are highly technical within the medical context. For example: `range motion` and `dressing applied` and found within `Orthopedic` specialty. Similarly, `motor units` is found within the `Radiology` specialty. Hence, we need a special or dedicated medical lemmatizer to correctly generate the lemmas of the intricate or specific medical terms.**
...
**In case of the scatter plot between Surgery and Orthopedic, the density of the distribution of the lemmas is spread along the regression line, indicating that the lemmas between the Orthopedic and Radiology specialty are relatively similar. On the other hand, the lemmas between Radiology and Surgery are not relatively similar, based on the density of scatter plot and the spread of distribution, away from the regression line.**

**The scatter plot between Surgery and Orthopedic and Radiology and Surgery is justifiable because, most of the medical procedures in the Orthopedic specialty involve Surgery along with Radiology. In case of Radiology, the samples could be used for diagnostics purposes, not necessarily involving surgery.**
...
**Answer: Within the `Surgery` specialty, we come across a lemma `anesthesia endotracheal` with `168 counts`. Alongside, we also come across the same lemma under `Surgery` which is put the other way round, i.e., `endotracheal anesthesia` with `167 counts`.**

**The other lemmas, reflect on their respective specialty. Some lemmas even intersect among specialties, for example, `steri strips`, intersects with `Orthopedic` and `Surgery`.**

**However, for `anesthesia endotracheal` and `endotracheal anesthesia`, we need contextual relevance to justify the position of the wording.**
...
**Answer: Within the `Surgery` specialty, we obtain `anesthesia endotracheal` and `endotracheal anesthesia` and look up the transcriptions for their context.**

```{r}
analysis.data %>% dplyr::select(medical_specialty, transcription) %>% dplyr::filter(stringr::str_detect(transcription, 'anesthesia endotracheal')) %>% dplyr::slice(1)
```

```{r}
analysis.data %>% dplyr::select(medical_specialty, transcription) %>% dplyr::filter(stringr::str_detect(transcription, 'endotracheal anesthesia')) %>% dplyr::slice(1)
```
...
**The 3 topic LDA is implemented as follows:**
...
**We see that the lemma prepped draped appears as the top term in all the 3 topics.**

**The 6 topic LDA is implemented as follows:**
...
**We see the top terms from the 3 topic LDA i.e., `prepped draped` still shows in 5 of the 6 topics. However, in the sixth topic, the term `prepped draped` appears as the second term, with `coronary artery` being the top term.**

**By the definition of LDA, each medical specialty could be considered as a document. Each specialty contains a number of words which are within the scope of that specialty. Based on the number of topics (sub-topics) entered, LDA uses probability to iteratively assign each word to a particular topic.**
